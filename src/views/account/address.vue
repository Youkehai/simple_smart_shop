                    ...
                    <div v-if="receiveInfo.length">
                      <div class="address-list-item default-item" v-for="(receive,index) in receiveInfo" :key='index'>
                        <div class="name fn-left">
                            <div class="name-cell">{{receive.consigneeName}}</div>
                        </div>
                        <div class="detail fn-left">
                            <div class="detail-cell">{{receive.consigneePhone}}&nbsp;   {{receive.province}}   {{receive.city}}   {{receive.place}}   {{receive.address}}</div>
                        </div>
                        ...
                        <div class="default fn-right">
                          <span v-if="receive.isDefault=='1'">（默认地址）</span>
                  ...
                  <!--      <div class="module-form-row">
                        ...
                        <div class="module-form-row clear">
                            <div class="form-item-v3 select-item province-wrapper">
                                <select name="province_code" class="province select-province js-form-province js-verify" @change="changePro">
                                    <option value="0">请选择省份</option>
                                    <option :value="item.id" v-for="item in proList" :key="item.provId">{{item.name}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="module-form-row clear">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkDefaultHandle (a) {
		  var b={
			  id:a.id,
			  isDefault:'1'
		  }
		  let that=this
		  y.put('/api/jwt/address/address',b,data=>{
			 if(data && data.success){
				y.success("修改成功") 
				that.getReceiveInfo()
			 }else{
