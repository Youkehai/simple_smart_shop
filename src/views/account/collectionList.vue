			...
			<div class="js-list-inner">
				<div class="box-inner order-cart order-list-cart clear" v-for="(order,index) in orderData" :key='index'>
					<div class="gray-sub-title cart-title">
						...
						<span class="order-id"> 商户名称： <a href="javascript:;">{{order.name}}</a> </span>
						<span class="order-detail">
							<span v-if="order.status==1" class="blue-small-btn js-payment-order" @click="goBuiness(order)">前往店铺</span>
					  </span>
					</div>
				</div>
			</div>
...
export default{
	...
	methods:{
		goBuiness(order){//进店逛逛
				  //设置一个值，改变头部展示
			  y.setVal("showType","buiness");
			  y.setVal("buinessName",order.name);
			  this.$store.commit('setShowType',"buiness")
			  this.$store.commit('setBuinessName',order.name)
			  y.setVal("buinessId",order.id);
			  //跳转页面
			 y.go('/buinessShop')
		},
		getAllOrder(){
			let that=this
			y.get('/api/jwt/collection/collection/'+JSON.parse(y.getVal("userInfo")).id,{},data=>{
