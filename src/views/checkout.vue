  				...
  				<div class="box-inner js-checkout-address-panel ">
  					<div class="address-common-table js-multiple-address-panel">
  						<ul class="address-item-list clear js-address-item-list">
  							<li class="js-choose-address" @click="receiveIndex=index" :class="{'selected-address-item':receiveIndex == index}" v-for="(item,index) in receiveInfo" :key='index'>
  								<div class="address-item">
  									<div class="name-section">  {{item.consigneeName}}  </div>
  									<div class="mobile-section">{{item.consigneePhone}}</div>
  									<div class="detail-section"> {{item.province}} {{item.city}} {{item.place}}<br> {{item.address}} </div>
  								</div>
  ...
  <!-- 		<div class="gray-box">
  				...
  				<div class="invoice-detail"> 发票抬头：
  					<div class="radio-box">
  						<label>
  							...
  							<span class="blue-radio" @click="checkInvoice(true)" :class="{'blue-radio-on' : invoice.personal}"><a></a></span>  个人
  						</label>
  						<label>
  							<input type="radio" class="hide">
  							<span class="blue-radio" @click="checkInvoice(false)" :class="{'blue-radio-on' : !invoice.personal}"><a></a></span>  单位
  						</label>
  					</div>
  		...
  		<div class="gray-box">
  			...
  			<div class="box-inner ui-goods-cart">
  				...
  				<div class="cart-table">
  					<div class="cart-group js-cart-group">
  						<div class="cart-items" v-for="(item,index) in this.$store.state.orderData.itemList" :key='index'>
                    ...
                    <img :src="item.images"></a>
  							...
  							<div class="subtotal">
  								<div class="subtotal-cell">  ¥ {{item.price/item.num}}.00  </div>
  							</div>
  			...
  			<div class="box-inner">
				...
				<el-input placeholder="请输入订单备注" v-model="tips" type="textarea" style="margin-bottom: 10px;"/>
  					<p> 商品总计： <span>¥ {{this.$store.state.orderData.price}}.00</span> </p>
            ...
            <span class="prices fn-right">应付金额： <em>¥ {{this.$store.state.orderData.price}}.00</em></span>
  				</div>
  			</div>
  		</div>
  	</div>
...
<script>
  ...
  export default{
    ...
    created(){
		//this.shopList=this.$store.state.orderData.itemList
		this.getReceiveInfo()
    },
    methods:{
	getReceiveInfo(){
	  let that=this
	  y.get('/api/jwt/address/address',{},data=>{
		  if(data && data.success){
			  that.receiveInfo=data.data
      ...
      //提交订单
      submitOrder(){
		  if(!this.$store.state.orderData.itemList || !this.$store.state.orderData.itemList.length || this.$store.state.orderData.itemList.length==0){
			  y.error("请重新选取物品")
			  return false
		  }
		  var order=this.$store.state.orderData
		  var item=this.receiveInfo[this.receiveIndex]
		  //拿到所有的商品，根据商户ID分组，分成几个组就新增几个订单
		  var itemList=order.itemList
		  var itemMap=new y.HashMap()
		  for(var i=0,len=itemList.length;i<len;i++){
			  if(itemMap.get(itemList[i].buinessId)){
				var list=itemMap.get(itemList[i].buinessId)
				list.push(itemList[i])
				 itemMap.put(itemList[i].buinessId,list)
			  }else{
				  var list=[]
				  list.push(itemList[i])
				  itemMap.put(itemList[i].buinessId,list)
		  ...
		  var orderList=[]
		    for(var map of itemMap.keySet()){
				var list=itemMap.get(map)
				var len=list.length
				var num=0
				var price=0
				for(var i=0;i<len;i++){
				...
				var result={
					...
					consigneeAddress:item.province+item.city+item.place+item.address,
		  ...
		  // order.consigneeName=item.consigneeName
		  // order.consigneeAddress=item.province+item.city+item.place+item.address
		  // order.consigneePhone=item.consigneePhone
		  // order.buinessId=order.itemList[0].buinessId
		  // order.tips=this.tips
		  let that=this
		  y.post('/api/jwt/order/order',orderList,data=>{
		  	if(data && data.success){
		  		y.success("下单成功")
				var len=data.data.length
				var ids=""
