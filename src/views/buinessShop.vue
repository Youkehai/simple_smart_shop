  ...
  <div id="main">
  	<div class="sku-box store-content ">
  		<div class="sort-option">
  			<ul class="line clear">
  				<li><a href="javascript:;" class="active" @click="getShopList(false,'')">综合排序</a></li>
  				<li><a href="javascript:;" class="" @click="getShopList(true,'price')">价格低到高</a></li>
  				<li><a href="javascript:;" class="" @click="getShopList(false,'price')">价格高到低</a></li>
				<el-input style="float: right;width:30%;" v-model="page.name" placeholder="请输入商品进行搜索" @keyup.enter.native="getShopList(false,'')"></el-input>
  			</ul>
  		</div>
  		<div class="gray-box">
        <header class="title-header">
        	<h5 class="title">{{this.$route.query.name}}</h5>
  			...
  			<div class="item-box">
  				<shopItem v-for="(item,index) in goodList" :key='index' :item='item'></shopItem>
  			</div>
			<el-pagination
...
<script>
  ...
  import shopOrder from '../components/shopOrderList.vue'
export default {
	data() {
		return {
			goodsData,
			page:{
				...
				isAsc:false,
				orderClounm:'',
				shopTypeId:this.$route.query.typeId,
				total:0,
				buinessId:y.getVal("buinessId"),
				name:''
			},
			goodList:[],
		}
  ...
  watch:{
	  "$route"(val){
		  this.page.shopTypeId=val.query.typeId
		  this.getShopList()
	  }
  },
  methods:{
	  ...
	  getShopList(isAsc,orderClounm){
		  let that=this
		  that.goodList=[]
		  this.page.isAsc=isAsc
		  this.page.orderClounm=orderClounm
		  y.get('/api/open/index/shop',this.page,data=>{
			  if(data && data.success){
				  that.goodList=data.data.records
