  ...
  <div id="main" class="hander-car">
  	<div class="store-content">
  		<div class="cart-box">
  							...
  							<div class="cart-top-items" v-for="(item,index) in carPanelData" :key='index'>
  								<div class="cart-items">
  									<div class="items-choose">
  										<span class="blue-checkbox-new" @click="checkGoods(item.id,index)" :class="{'checkbox-on':item.check}"><a></a></span>
  									</div>
                        ...
                        <img :src="item.imagesList">
  									...
  									<div class="name hide-row" >
  										<div class="name-table">
  											<a href="javascript:;" target="_blank">{{item.shopInfo.name}}</a>
  											<ul class="attribute">
  									...
  									<div class="operation">
  										<a class="items-delete-btn" @click="delCarPanelData(item.id)"></a>
  									</div>
  									<div class="subtotal">¥ {{item.price}}</div>
  									<div class="item-cols-num">
  										<div class="select js-select-quantity">
  											<span class="down" :class="{'down-disabled':item.num <= 1}" @click="subCartPanelData(item,index)">-</span>
  											<span class="num">
  												{{item.num}}
  											</span>
  											<span class="up"  @click="plusCartPanelData(item,index)">+</span>
  				...
  				<div class="cart-bar-operation">
  					<div>
  						<div class="choose-all js-choose-all">
  							<span class="blue-checkbox-new" @click="allGoodsCheck(allChecked)" :class="{'checkbox-on':allChecked}"><a></a></span>
  							全选
  						</div>
            ...
            <div tag="span" @click="addOrder" class="jianguo-blue-main-btn big-main-btn js-checkout" :class='{"disabled-btn":checkCount<=0}'>
              <a>现在结算</a>
            </div>
  				</div>
  			</div>
...
<script>
  export default{
    ...
    methods:{
		addOrder(){//点击现在结算，新增一个代付款的订单
			var itemList=[]
			var len=this.carPanelData.length
			for(var i=0;i<len;i++){
				if(this.carPanelData[i].check){
					var itemData={
						carId:this.carPanelData[i].id,
						images:this.carPanelData[i].imagesList,
						shopId:this.carPanelData[i].shopInfo.id,
						shopName:this.carPanelData[i].shopInfo.name,
						shopSpecifications:this.carPanelData[i].specifications,
						num:this.carPanelData[i].num,
						buinessId:this.carPanelData[i].shopInfo.buinessId,
						price:this.carPanelData[i].price
					}
	...
	getCarPanlData(){//获取购物车数据
	  let that=this
	  if(y.isLogin("/","skip")){
		  y.get('/api/jwt/shopCar/shopCar?sdads=123',{},data=>{
      ...
      plusCartPanelData(item,index){
		  ...
		  let that=this;
		   y.put('/api/jwt/shopCar/shopCar',shop,data=>{
			  if(data && data.success){
				  that.getCarPanlData()
			  }
		   })
    //     this.$store.commit('plusCartPanelData',item.id)
      },
      //减少
      subCartPanelData(id,index){
		  price=item.price/item.num
		  var shop={
			  id:item.id,
			  num:item.num-=1,
      ...
      checkGoods(id,index){
		  ...
		  this.carPanelData[index].check=!this.carPanelData[index].check
		  this.checkPrice=0
		  this.checkCount=0
		  for(var i=0,len=this.carPanelData.length;i<len;i++){
			  if(this.carPanelData[i].check){
				  this.checkPrice+=Number(this.carPanelData[i].price)
				  this.checkCount+=Number(this.carPanelData[i].num)
			  }
		  }
		  let arr = this.chooseIds.filter(o => o== id);
      },
      //控制全选
      allGoodsCheck(allChecked){
		  this.allChecked=!allChecked
		  var len=this.carPanelData.length
		  if(this.allChecked){
			  for(var i=0;i<len;i++){
				  this.carPanelData[i].check=true
				  this.chooseIds.push(this.carPanelData[i].id)
			  }
		  }else{//如果取消全选，去除所有ID
			 this.chooseIds=[]
			 for(var i=0;i<len;i++){
				  this.carPanelData[i].check=false
			 }
		  }
      },
      //删除选中的商品
